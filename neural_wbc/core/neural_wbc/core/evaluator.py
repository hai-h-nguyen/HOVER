...
class Evaluator:
    ...
    def visualize(self, dt):
        ...
        num_plot_for_each_joint = 3
        nb_rows = nb_cols = math.ceil(math.sqrt(num_joints * num_plot_for_each_joint + 3))
        fig, axs = plt.subplots(nb_rows, nb_cols)
        time = np.linspace(0, _num_frames * dt, _num_frames)

        for i in range(0, num_joints * num_plot_for_each_joint, num_plot_for_each_joint):
            row = (i) // nb_rows
            col = (i) % nb_cols
            a = axs[row, col]
            joint_index = int(i / num_plot_for_each_joint)
    ...
    def _build_frame(
        ...
        """
        new_data = {}
        new_data["body_pos"] = data["body_pos"]
        new_data["body_pos_masked"] = data["body_pos"] * mask
        joint_pos = data.pop("joint_pos")
        new_data["upper_body_joint_pos"] = joint_pos[:, upper_joint_ids]
        new_data["lower_body_joint_pos"] = joint_pos[:, lower_joint_ids]
        new_data["root_pos"] = data["root_pos"]
