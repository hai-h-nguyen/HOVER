...
class Recorder:
    ...
    def __init__(self, env_wrapper: EnvironmentWrapper, log_path: str):
    ...
    def collect(self, dones: torch.Tensor, info: dict) -> bool:
        ...
        if self.frames is None:
            ...
            self.frames = self.initialize_frame(
                self._ref_motion_mgr.motion_lib._curr_motion_ids.item(), self._ref_motion_frames
            )
        if self.frames["mask"] is None:
            self.frames["mask"] = info["data"]["mask"].cpu().numpy()
        self.frames["robot_state"]["root_pos"][self._curr_steps, :] = (
            info["data"]["recorded_state"]["root_pos"].cpu().numpy()
        )
        self.frames["robot_state"]["root_lin_vel"][self._curr_steps, :] = (
            info["data"]["recorded_state"]["root_lin_vel"].cpu().numpy()
        )
        self.frames["robot_state"]["root_ang_vel"][self._curr_steps, :] = (
            info["data"]["recorded_state"]["root_ang_vel"].cpu().numpy()
        )
        self.frames["robot_state"]["root_rot"][self._curr_steps, :] = (
            info["data"]["recorded_state"]["root_rot"].cpu().numpy()
        )
        self.frames["robot_state"]["joint_pos"][self._curr_steps, :] = (
            info["data"]["recorded_state"]["joint_pos"].cpu().numpy()
        )
        self.frames["robot_state"]["joint_vel"][self._curr_steps, :] = (
            info["data"]["recorded_state"]["joint_vel"].cpu().numpy()
        )
        ...
        if (self._curr_steps >= self._ref_motion_frames) or dones[0]:
            ...
            self.frames["robot_state"]["root_lin_vel"] = self.frames["robot_state"]["root_lin_vel"][
                : self._curr_steps, :
            ]
            self.frames["robot_state"]["root_ang_vel"] = self.frames["robot_state"]["root_ang_vel"][
                : self._curr_steps, :
            ]
        ...
        with h5py.File(log_path, "w") as f:
            ...
            for idx, item in enumerate(self._recorded_frames_total):
                ...
                robot_state_group.create_dataset(
                    "root_lin_vel", data=item["robot_state"]["root_lin_vel"], compression="gzip"
                )
                robot_state_group.create_dataset(
                    "root_ang_vel", data=item["robot_state"]["root_ang_vel"], compression="gzip"
                )
