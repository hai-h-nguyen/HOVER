...
class PolicyCfg:
    ...
    init_noise_std: float = field(default=0.8, metadata={"description": "Initial noise standard deviation for policy."})
    ...
    # )
    actor_hidden_dims: List[int] = field(
        default_factory=lambda: [64, 32], metadata={"description": "Hidden dimensions for the actor network."}
    )
    critic_hidden_dims: List[int] = field(
        default_factory=lambda: [64, 32], metadata={"description": "Hidden dimensions for the critic network."}
    )
...
class AlgorithmCfg:
    ...
    entropy_coef: float = field(default=0.01, metadata={"description": "Coefficient for entropy regularization."})
    ...
    max_grad_norm: float = field(default=1.0, metadata={"description": "Maximum norm for gradient clipping."})
...
class RunnerCfg:
    ...
    path: str = field(default="logs/almi", metadata={"description": "Directory to save policy and logs to"})
    policy_class_name: str = field(default="RNNActorMLPCritic", metadata={"description": "Class name for the policy."})
...
class ALMIPolicyCfg:
    ...
    @staticmethod
    def load(filepath: str) -> "ALMIPolicyCfg":
        """Loads the configuration from a YAML file."""
        with open(filepath, encoding="utf-8") as file:
            data = json.load(file)
        cfg = fromdict(ALMIPolicyCfg, data)
        return cfg
    ...
    def add_args_to_parser(parser: argparse.ArgumentParser, default_overwrites: dict = {}):
        ...
        def add_fields_to_parser(fields):
            for field_info in fields:
                arg_name = f"--{ALMIPolicyCfg.args_prefix()}{field_info.name}"
        ...
        add_fields_to_parser(f for f in fields(ALMIPolicyCfg) if not is_dataclass(f.type))
        add_fields_to_parser(fields(PolicyCfg))
        add_fields_to_parser(fields(AlgorithmCfg))
        add_fields_to_parser(fields(RunnerCfg))

    @staticmethod
    def from_argparse_args(args: argparse.Namespace) -> "ALMIPolicyCfg":
        ...
        def extract_fields(cls: Type) -> dict:
            ...
            extracted_fields = {
                field.name: getattr(args, ALMIPolicyCfg.args_prefix() + field.name)
                for field in fields(cls)
                if hasattr(args, ALMIPolicyCfg.args_prefix() + field.name)
            }
        ...
        return ALMIPolicyCfg(
            seed=getattr(args, f"{ALMIPolicyCfg.args_prefix()}seed"),
    ...
    def overwrite_policy_cfg_from_file(self, file_path: str):
        try:
            saved_config = ALMIPolicyCfg.load(file_path)
