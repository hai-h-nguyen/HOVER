...
parser = argparse.ArgumentParser(
    description="Train RL-based delta action agent with RSL.",
...
def main():
    ...
    if args_cli.robot == "h1":
        env_cfg = NeuralWBCEnvCfgH1(mode=NeuralWBCModes.DELTA_ACTION)
    elif args_cli.robot == "h12":
        env_cfg = NeuralWBCEnvCfgH12(mode=NeuralWBCModes.DELTA_ACTION)
    elif args_cli.robot == "g1":
        env_cfg = NeuralWBCEnvCfgG1(mode=NeuralWBCModes.DELTA_ACTION)
    ...
    delta_action_policy_cfg = TeacherPolicyCfg.from_argparse_args(args_cli)
    ...
    if delta_action_policy_cfg.runner.resume:
        ppo_runner, checkpoint_path = get_ppo_runner_and_checkpoint_path(
            teacher_policy_cfg=delta_action_policy_cfg, wrapped_env=env, device=env.unwrapped.device, log_dir=log_dir
        )
    ...
    env.seed(delta_action_policy_cfg.seed)

    # run training
    ppo_runner.learn(num_learning_iterations=delta_action_policy_cfg.runner.max_iterations, init_at_random_ep_len=False)
