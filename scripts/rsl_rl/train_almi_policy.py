...
def main():
    ...
    if args_cli.robot == "h12":
        env_cfg = NeuralWBCEnvCfgH12_ALMI()
    else:
        raise ValueError("Your robot is not yet implemented")

    env_cfg.scene.num_envs = args_cli.num_envs
    env_cfg.scene.env_spacing = 20
    env_cfg.terrain.env_spacing = 20

    almi_policy_cfg = ALMIPolicyCfg.from_argparse_args(args_cli)

    # Create env and wrap it for RSL RL.
    env = ALMIEnv(cfg=env_cfg)
    env = RslRlNeuralWBCVecEnvWrapper(env)

    # specify directory for logging experiments
    log_dir = os.path.join(os.path.abspath(almi_policy_cfg.runner.path), datetime.now().strftime("%y_%m_%d_%H-%M-%S"))
    os.makedirs(log_dir, exist_ok=True)
    print(f"[INFO] Log directory: {log_dir}")

    if almi_policy_cfg.runner.resume:
        ppo_runner, checkpoint_path = get_ppo_runner_and_checkpoint_path(
            almi_policy_cfg=almi_policy_cfg, wrapped_env=env, device=env.unwrapped.device, log_dir=log_dir
        )
        ppo_runner.load(checkpoint_path)
        print(f"[INFO]: Loaded model checkpoint from: {checkpoint_path}")
    else:
        ppo_runner = OnPolicyRunner(env, almi_policy_cfg.to_dict(), log_dir=log_dir, device=env.unwrapped.device)

    # Store the configuration
    almi_policy_cfg.save(os.path.join(log_dir, "config.json"))
    env_cfg.save(os.path.join(log_dir, "env_config.json"))

    # set seed of the environment
    env.seed(almi_policy_cfg.seed)

    # run training
    ppo_runner.learn(num_learning_iterations=almi_policy_cfg.runner.max_iterations, init_at_random_ep_len=True)
    ...
    #     # simulate physics
    # while num_demos < 10000 * env_cfg.scene.num_envs:
