...
class Humanoid_Batch:
    ...
    def __init__(self, mjcf_file = f"resources/robots/g1/g1_29dof_anneal_23dof_fitmotionONLY.xml", extend_hand = True, extend_head = False, device = torch.device("cpu")):
        ...
        if self.extend_hand:
            self.body_names_augment += ['left_hand_link', 'right_hand_link']
            self._parents = torch.cat([self._parents, torch.tensor([19, 23]).to(device)], dim=0)
            self._offsets = torch.cat((self._offsets, torch.tensor([[[0.25, 0, 0], [0.25, 0, 0]]])), dim = 1).to(device)
            self._local_rotation = torch.cat((self._local_rotation, torch.tensor([[[1, 0, 0, 0], [1, 0, 0, 0]]])), dim = 1).to(device)
            self.num_extend_dof += 2

        if self.extend_head:
            self.body_names_augment += ['head_link']
            self._parents = torch.cat([self._parents, torch.tensor([15]).to(device)], dim=0)
            self._offsets = torch.cat((self._offsets, torch.tensor([[[0, 0, 0.42]]])), dim = 1).to(device)
            self._local_rotation = torch.cat((self._local_rotation, torch.tensor([[[1, 0, 0, 0]]])), dim = 1).to(device)
...
if __name__ == "__main__":
    hb = Humanoid_Batch(mjcf_file="/home/rtx4/multi_mode_control/resources/robots/g1/g1_29dof_anneal_23dof_fitmotionONLY.xml", extend_head=True)
    hb.mesh_fk()